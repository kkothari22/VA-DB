...
class BMDriver extends TestDriver implements GlobalConst {
	...
	public boolean runTests() {
		...
		try {
			// My comments : --- Params: DatabaseName, No of Pages, No of
			// Buffers, Replacement Policy from buffer
	...
	protected boolean test1() {
		...
		boolean status = OK;
		// My comment ---> getNumUnpinnedBuffers() returns number of available
		// empty frames in buffer
		int numPages = SystemDefs.JavabaseBM.getNumUnpinnedBuffers() + 1;
		System.out.println("*********** Initially num of Pages ---->"
				+ numPages);
		Page pg = new Page();
		// My comment: PageId object contains just int pageId
		PageId pid;
		PageId lastPid;
		// Object creation not necessary
		PageId firstPid = new PageId();

		System.out.print("  - Allocate a bunch of new pages\n");

		try {
			// My comment --> 'SystemDefs.JavabaseBM' gives instance of
			// BufferMgr and internally pins the Page to the Buffer
		...
		// My comment : UnPin implies the page becomes read for removal from
		// buffer. This doesnt mean page is removed.
		...
		for (pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages; status == OK
			...
			if (status == OK) {
				...
				try {
					// My comment --> sets integer(data) at position(0) on
					// page(pg)
		...
		for (pid.pid = firstPid.pid; status == OK && pid.pid < lastPid.pid; pid.pid = pid.pid + 1) {
			...
			try {
				// Check if this page is in buffer pool, otherwise find a frame
				// for this page, read in and pin it.
			...
			if (status == OK) {
				...
				try {
					// My comment --> reads 4 bytes(bcoz its an integer) from
					// given position(0) on page(pg)
					...
					if (pid.pid % 20 != 12) {
						try {
							SystemDefs.JavabaseBM.unpinPage(pid, /* dirty: */
									true);
							System.out.print ("Being unPinned   " + pid.pid + "      ");
						} catch (Exception e) {
							status = FAIL;
							System.err.print("*** Could not unpin dirty page "
									+ pid.pid + "\n");
		...
		if (status == OK) {
			...
			for (index = 0; status == OK && index < numPages; ++index) {
				...
				try {
					...
					System.out.print("Being pinned  " + pid.pid);
				} catch (Exception e) {
					status = FAIL;
					System.err
				...
				if (status == OK && (pid.pid % 20 == 12)) {
					try {
						...
						System.out.println("Being unpinned Again   " + pid.pid);
					} catch (Exception e) {
						status = FAIL;
						System.err.print("*** Could not unpin page " + pid.pid
								+ "\n");
