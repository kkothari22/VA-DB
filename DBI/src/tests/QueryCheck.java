...
public class QueryCheck {
	...
	public QueryCheck(int q_index) {
		...
		case 1:
			...
			mygroup[0].order = new Order(Order.UNSORT); // the tuple in group is
														// sorted or not
			for (int i = 0; i < mygroup[0].len; i++) {// set tuple value.
				try {
					...
					mygroup[0].mytuple[i].setStrFld(1,
							((S1) Q1result.elementAt(i)).sname);
					mygroup[0].mytuple[i].setStrFld(2,
							((S1) Q1result.elementAt(i)).date);
				} catch (Exception e) {
		...
		case 5:
			...
			for (int i = 0; i < mygroup[0].len; i++) {// set tuple value.
				try {
					...
					mygroup[0].mytuple[i].setHdr(columnum, types, sizes);
					mygroup[0].mytuple[i].setStrFld(1,
							((S5) Q5result.elementAt(i)).sname);
					mygroup[0].mytuple[i].setIntFld(2,
							((S5) Q5result.elementAt(i)).rating);
					mygroup[0].mytuple[i].setFloFld(3,
							((S5) Q5result.elementAt(i)).age);
				} catch (Exception e) {
					System.err.println("**** Error setting up the tuples");
				}
			}
	...
	void TupleCopy(Tuple to, Tuple from, int fldnum, AttrType[] type) {
		...
		for (int i = 1; i <= fldnum; i++) {
			...
			case AttrType.attrString:
				...
				break;
			default:
				// error(Don't know what to do with attrSymbol,
				// attrNull--TupleCopy(..);
				break;
			}
		}
	...
	public void Check(Tuple t) {
		...
		if (mygroup[curGroup].order.order == Order.SORT) {
			...
			try {
				...
				if (tUtil.Equal(mygroup[curGroup].mytuple[count], t, types,
						(int) columnum)) {
			...
			for (int i = 0; i < mygroup[curGroup].len; i++) {
				try {
					...
					if ((mygroup[curGroup].mark[i] == 0)
							&& (tUtil.Equal(mygroup[curGroup].mytuple[i], t,
									types, (int) columnum))) {// found
					...
					System.err.println("" + e);
					System.err
							.println("***** Error comparing the value of tuples");
				}
			}
	...
	void MisMatch(Tuple t) {
		...
		if (tempGroup == -1) { // t not in answer tuples
			...
			// if mygroup[curGroup].count == 0, it's probably due to groups are
			// not
			...
			if (grouporder.order == Order.SORT) {
				...
				if (tempGroup != curGroup) {
					...
					// change the grouporder to UNSORT to facilitate further
					// checking
			...
			// tuple should be sorted, check whether it's the first tuple of
			// group
			else if (t_num[0] == 0) {
				MarkTuple(curGroup, t_num[0]);
			}
				...
				for (int j = 0; j < mygroup[i].len; j++) {
					try {
						...
						if ((mygroup[i].mark[j] == 0)
								&& (tUtil.Equal(mygroup[i].mytuple[j], t,
										types, (int) columnum))) {
							t_num[0] = j;
							return i;
						}
